(function(){
  ...
  function page(snapshot,shared=false){
    ...
    return `<div class="page-shell career-page">
      ...
      <article class="career-sheet" aria-label="${esc(s.name)} 的完整生涯">
        ...
        <section class="career-section career-seasons"><div class="career-section-title"><h2><span>02</span> 十五届征程</h2><small><i class="career-change-dot"></i> 新加入成员</small></div><div class="career-table-scroll" tabindex="0" role="region" aria-label="逐届名次、当届冠军与阵容，可横向滚动"><table class="career-table"><thead><tr><th scope="col">赛事</th><th scope="col">名次</th><th scope="col">当届冠军</th>${C.ROLES.map(r=>`<th scope="col">${r}</th>`).join('')}</tr></thead><tbody>${s.events.map((e,i)=>`<tr class="${e.placement==='1'?'won-title':''}"><th scope="row">${C.eventName(e.year)}</th><td><div class="career-place">${e.placement==='1'?aegis(e.year):''}<span>${C.placeName(e.placement)}</span></div></td><td class="career-season-champion"><strong>${esc(e.champion||'未记录')}</strong></td>${e.seats.map((id,j)=>{const m=s.members[id],changed=C.changed(s,i,j);return `<td class="${changed?'member-joined':''}"><strong>${changed?'<span class="sr-only">新加入：</span>':''}${esc(m.name)}</strong><small>${C.eventName(m.year)}${changed?' · 新加入':''}</small></td>`;}).join('')}</tr>`).join('')}</tbody></table></div></section>
  ...
  async function poster(snapshot){
    ...
    function wrap(text,width,size,weight=400,words=false){
      setFont(size,weight);const lines=[];let row='';
      for(const c of [...String(text)]){
        if(row&&ctx.measureText(row+c).width>width){
          const space=words?row.lastIndexOf(' '):-1;
          if(space>0){lines.push(row.slice(0,space));row=row.slice(space+1)+c;}
          else{lines.push(row);row=c;}
        }else row+=c;
      }
    ...
    const widths=[60,102,150,112,112,112,112,112,112],xs=[48];
    widths.forEach((w,i)=>xs.push(xs[i]+w));
    const rows=s.events.map(e=>{const lines=e.seats.map(id=>wrap(s.members[id].name,96,20,600)),champion=wrap(e.champion||'未记录',134,19,600,true);return {lines,champion,height:Math.max(78,Math.max(...lines.map(l=>l.length))*27+43,champion.length*27+30)};});
    ...
    ['赛事','名次','当届冠军',...C.ROLES].forEach((v,i)=>text(v,xs[i]+8,y+12,15,muted,500));y+=43;
    s.events.forEach((e,i)=>{
      ...
      row.champion.forEach((l,k)=>text(l,xs[2]+8,y+15+k*27,19,e.champion?gold:muted,600));
      e.seats.forEach((id,j)=>{const m=s.members[id],changed=C.changed(s,i,j),x=xs[j+3]+8;row.lines[j].forEach((l,k)=>text(l,x,y+15+k*27,20,changed?accent:ink,600));text(C.eventName(m.year)+(changed?' · 新':' '),x,y+15+row.lines[j].length*27+5,15,changed?accent:muted);});
      y+=row.height;rule(y);
    });
