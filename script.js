(function(){
  ...
  function matchReport(r,g){
    ...
    return `<div class="map-overview"><strong>${escape(name(winner))} 胜利</strong><span>${g.minutes} 分钟</span><span>击杀 ${g.kills.join(' : ')}</span><span>防御塔 ${g.towers.join(' : ')}</span></div>${sides}<div class="bp-ban-list"><span>禁用</span>${g.draft.banned.map(h=>heroPortrait(h)).join('')}</div><p class="report-coaches">Coach · ${g.draft.coaches.map((coach,i)=>escape(coach||'默认战术')+'（'+escape(name(i?g.b:g.a))+'）').join(' / ')}</p><details class="report-events"><summary>关键推进</summary><ul class="event-list">${g.events.slice(-5).map(e=>`<li><time>${e.minute}:00</time><span>${escape(name(e.side===0?g.a:g.b))} · ${e.text}</span></li>`).join('')}</ul></details>`;
  }
