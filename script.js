(function(){
      ...
      <div class="home-content">
        ...
        <section class="home-how" aria-labelledby="home-how-title">
          ...
          <div class="home-steps">
            ...
            <article><span class="home-step-index">03</span>${aegis(D.years.at(-1),'home-aegis')}<h3>征战 TI，续写你的生涯</h3><p>替代当届八强末队，直接进入双败淘汰赛。回看每局 BP 和战报。休赛期换一人，最多携手征战十届。</p><span>THE LEGACY / 生涯挑战</span></article>
          </div>
        </section>
  ...
  function result(){
    const r=state.history.at(-1),path=r.bracket.filter(m=>m.a==='myteam'||m.b==='myteam'),final=r.bracket.at(-1),canNext=G.canAdvance(state);
    return `<div class="page-shell">${crumbs('赛事结果')}<div class="page-heading"><div><div class="eyebrow">THE ROAD TO THE AEGIS / ${r.year}</div><h1>${r.placement==='1'?'这一届，属于你的战队。':'每一场，都是传奇的一部分。'}</h1><p>${eventName(r.year)} · 八强双败淘汰赛 · BO3 / 总决赛 BO5</p></div>${steps(2)}</div>
      ${teamIdentity()}
      <section class="result-hero"><div><div class="eyebrow">${r.placement==='1'?'AEGIS OF CHAMPIONS':'YOUR TOURNAMENT RESULT'}</div><h2>${placementLabel(r.placement)}</h2><p>${escape(r.teamName)}${r.placement==='1'?' · 捧起属于你们的冠军盾。':` · 本届冠军：${escape(teamName(r.champion))}`}</p><div class="result-facts"><div><strong>${r.wins}<span class="muted"> — </span>${r.losses}</strong><small>小局胜负</small></div><div><strong>${Math.round(r.wins/(r.wins+r.losses)*100)}<small>%</small></strong><small>小局胜率</small></div><div><strong>${D.tactics[r.tactic].name}</strong><small>本届战术</small></div></div></div><div class="result-emblem ${r.placement==='1'?'is-champion':''}">${r.placement==='1'?aegis(r.year):icon('shield')}</div></section>
      <div class="result-columns"><section class="result-panel"><div class="section-heading"><h2>最终排名</h2><small>8 支战队</small></div><table class="standings"><thead><tr><th>名次</th><th>战队</th><th>胜</th><th>负</th></tr></thead><tbody>${r.standings.map(t=>`<tr class="${t.id==='myteam'?'my-row':''}"><td><span class="rank-number">${t.placement==='1'?aegis(r.year,'rank-aegis'):t.placement}</span></td><td>${escape(t.name)}${t.id==='myteam'?' · 我的战队':''}</td><td>${t.wins}</td><td>${t.losses}</td></tr>`).join('')}</tbody></table></section>
      <section class="result-panel"><div class="section-heading"><h2>我的淘汰赛之路</h2><small>${path.length} 场系列赛</small></div>${path.map(m=>{const side=m.a==='myteam'?0:1,won=m.winner==='myteam';return `<div class="match-row"><div class="opponent">${escape(teamName(side?m.a:m.b))}<small>${m.stage} · BO${m.bestOf}</small></div><div><div class="match-score ${won?'won':'lost'}">${m.score[side]} — ${m.score[1-side]}</div><span class="result-tag">${won?'VICTORY':'DEFEAT'}</span></div></div>`;}).join('')}<p class="fine-print" style="margin-top:16px">本届决赛：${escape(teamName(final.a))} ${final.score.join(' : ')} ${escape(teamName(final.b))}</p></section></div>
      ...
      <details class="result-details"><summary>逐局战报与 BP <span class="muted">/ ${r.games.length} 局</span></summary><div class="games-list">${r.games.map((g,i)=>{const side=g.a==='myteam'?0:1,won=g.winner===side;return `<button class="game-button" data-action="game-detail" data-game="${i}"><span>${escape(teamName(side?g.a:g.b))}<small>${g.stage} · 第 ${i+1} 局 · ${g.minutes} 分钟</small></span><span class="${won?'gold':'muted'}">${won?'胜利':'失利'} ${icon('chevron')}</span></button>`;}).join('')}</div></details>
      <details class="result-details"><summary>完整淘汰赛对阵 <span class="muted">/ ${r.bracket.length} 场系列赛</span></summary><div class="bracket-grid">${r.bracket.map(m=>`<div class="bracket-card"><small>${m.stage} · BO${m.bestOf}</small><div class="bracket-team ${m.winner===m.a?'winner':''}"><span>${escape(teamName(m.a))}</span><span>${m.score[0]}</span></div><div class="bracket-team ${m.winner===m.b?'winner':''}"><span>${escape(teamName(m.b))}</span><span>${m.score[1]}</span></div></div>`).join('')}</div></details>
  ...
  function gameDetail(index){
    ...
    const matchTeamName=id=>r.standings.find(t=>t.id===id)?.name||D.pools.find(p=>p.year===r.year&&p.team===id)?.name||id;
