(function(){
  ...
  function rules(){showModal(`<div class="eyebrow">HOW TO PLAY</div><h2 id="modal-title">五位传奇，一位名帅。</h2><ol class="rules-list"><li>每轮随机遇见一届 TI 的八强战队，选择一名选手。1—5 号位各一人。</li><li>五名选手齐备后，只抽 TI10—TI15 的年份，从当届八强各一名代表教练中选择第六席，可独立重抽年份 1 次。同一人跨届、跨选手和教练身份不能重复。</li><li>选手轮一次重抽同时更换年份和战队，整段生涯共 3 次。同一轮选人中，已经出现的战队不会再次抽中，包括其他年份的同队；选定一人后重新记录。教练轮只重抽年份，有独立的 1 次机会。没有合法候选不扣次数。</li><li>组队后确认阵容即可从 TI1（2011）出征，战队风格自动跟随教练，无需选择风格或英雄。自动 BP 参考历史样本；选手保留常用英雄头像，教练展示擅长风格和 TI 执教统计。</li><li>MyTeam 加当届八强共 9 队，自定义 BO2 循环赛、八强双败，决赛 BO5。不是复刻原届赛制。</li><li>一段生涯最多 10 届，在 TI2026 截止。每届结束可换一名选手或教练，下届生效。换教练同样只抽年份，可重抽 1 次；取消或切换位置不会重置。</li></ol><p class="fine-print">结果由游戏规则生成，不预测真实比赛。进度保存在当前浏览器。</p><div class="dialog-actions"><button class="primary" data-action="close-modal">明白了 ${icon('check')}</button></div>`);}
