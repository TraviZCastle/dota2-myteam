...
当前包含 TI1—TI15（2011—2019、2021—2026），每届决赛阶段最终前八名的五人名单，共 120 池、600 张选手版本卡；可选教练为 TI10—TI15 八强每队一位，共 48 张届次卡。不是所有 TI 参赛队的集合。运行时 `pools` 保留全部历史参赛名单；`draftPools` 和 `cards` 排除 TI1，仅向选秀、换人和图鉴提供 TI2—TI15 的 112 池、560 张选手卡及 48 张教练卡。
...
- 选手英雄池：该届 `heroUsage` 中使用次数大于 0 的全部英雄，不截取为卡面 Top 3；没有当届记录时参考本人已收录的选手生涯。备战不提供手动英雄选择。自动 BP 候选为位置英雄池与选手英雄池的并集；排除禁选与冲突后，两组均可用时 95% 概率从交集选取，5% 从其余候选选取。
- 教练战队胜率：可用样本中的该队胜局 / 局数。并列名次保存原区间。

TI1 没有可用 OpenDota 逐场快照：40 名历史对手 `stats=null`、`heroUsage={}`，不再作为可选卡展示。模拟的中性参数保存在运行时计算层，不写回历史统计。
